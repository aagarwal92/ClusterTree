...
d3.json("REDDITS3.json", function(json) {
    ...
    vis = d3.select("#chart").append("svg:svg").attr("width", w).attr("height", h).append("svg:g").attr("transform", "translate(100,0)");
...
function update(source) {
    ...
    }).on("mouseover", mouseon).on("mouseout", mouseoff).on("click", click).transition().duration(duration).attr("cx", function(d) {
        return d.y;
    }).attr("cy", function(d) {
        return d.x;
    });
    ...
    node.enter().append("svg:text").attr("dx", function(d) {
        return d.children ? -8 : 8;
    }).attr("dy", 3).attr("text-anchor", function(d) {
        return d.children ? "end" : "start";
    })
    ...
    node.exit().transition().duration(duration).attr("cx", function(d) {
        return source.y;
    }).attr("cy", function(d) {
        return source.x;
    }).remove();
    ...
    // Update the links…
    var link = vis.selectAll("path.link").data(cluster.links(nodes), function(d) {
        return d.target.id;
    });

    // Enter any new links at the parent's previous position.
    link.enter().insert("svg:path", "circle").attr("class", "link").attr("d", function(d) {
        var o = {
            x: source.x0,
            y: source.y0
        };
...
function resetNames() {
    ...
    var node = vis.selectAll("g.node").data(nodes).remove();
}
...
function resetSize(x) {
    ...
    cluster = d3.layout.cluster().size([h + 400, w - 240]);
    window.scrollTo(0, 0);
}
...
function showParentListBackButton() {
    ...
    for (i = 0; i < count; i++) {
        if (curNode2.parent != undefined) curNode2 = curNode2.parent;
        else
        curNode2 = curNode2;
    }
...
function mouseon(d) {
    ...
    tooltip.show('<strong> Sub-Reddit Name: </strong>' + d.name + '<br /><strong> Subscriber Count: </strong>' + d.size + '<br /><strong> Parent List: </stong>' + tooltipShow() + '<br /><strong> Children: </strong>' + tooltipKids());
}
